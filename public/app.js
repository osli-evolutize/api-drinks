...
function renderizarLista() {
  if (estado.receitas.length === 0) {
    els.lista.innerHTML = '<div class="loading">Nenhum drink encontrado.</div>';
    els.statusLista.textContent = "0 drinks";
    return;
  }

  els.statusLista.textContent = `${estado.receitas.length} drink${estado.receitas.length === 1 ? "" : "s"}`;
      ...
      <button class="recipe-card${ativa}" type="button" data-nome="${escaparHtml(receita.nome)}">
        ...
        <span>
          ...
          <span class="card-meta">
            ...
            <span>${escaparHtml(receita.pessoas)} pessoa${Number(receita.pessoas) === 1 ? "" : "s"}</span>
            <span>${escaparHtml(receita.volume || receita.peso)} ml</span>
          </span>
        </span>
      </button>
    `;
      ...
      <div class="hero-copy">
        ...
        <div class="recipe-title-row">
          ...
          <span class="recipe-title-actions">
            ${estado.autenticado ? `
              <a class="icon-button recipe-edit" href="nova.html?editar=${encodeURIComponent(receita.nome)}" aria-label="Editar drink" title="Editar drink">
            ...
            ` : ""}
            <button type="button" class="icon-button recipe-print" aria-label="Imprimir drink" title="Imprimir drink">
        ...
        <div class="stats" aria-label="Resumo do drink">
          <div class="stat"><strong>${escaparHtml(receita.pessoas)}</strong><span>Pessoas</span></div>
          <div class="stat"><strong>${escaparHtml(receita.volume || receita.peso)} ml</strong><span>Volume</span></div>
          <div class="stat"><strong>${escaparHtml(receita.alcool || 0)}%</strong><span>Alcool</span></div>
        </div>
      </div>
    </section>
      ...
      <div class="section-block">
        ...
        <div class="shopping-box" data-receita="${escaparHtml(receita.nome)}">
          ...
          <div class="shopping-controls">
            ...
            <button type="button" class="icon-button shopping-print" aria-label="Imprimir lista de ingredientes" title="Imprimir lista de ingredientes">
              <svg class="printer-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                <path d="M7 8V3h10v5"></path>
                <path d="M7 17H5a3 3 0 0 1-3-3v-3a3 3 0 0 1 3-3h14a3 3 0 0 1 3 3v3a3 3 0 0 1-3 3h-2"></path>
                <path d="M7 14h10v7H7z"></path>
...
function imprimirReceita(receita) {
  ...
  janela.document.write(`
    ...
    <html lang="pt-BR">
      <head>
        <meta charset="utf-8">
        <title>Drink - ${escaparHtml(receita.nome)}</title>
...
function imprimirListaCompras(receita, pessoasDesejadas) {
  ...
  janela.document.write(`
    ...
    <html lang="pt-BR">
      <head>
        <meta charset="utf-8">
        <title>Lista de ingredientes - ${escaparHtml(receita.nome)}</title>
        <link rel="icon" type="image/jpeg" href="images/chef-paulo.jpg">
        <style>
      ...
      <body>
        <h1>${escaparHtml(receita.nome)}</h1>
        <p>Lista de ingredientes para ${escaparHtml(pessoasDesejadas)} pessoa${Number(pessoasDesejadas) === 1 ? "" : "s"}</p>
        <ul>
          ${itens.map((item) => `
            <li>
...
async function carregarReceitas(selecionarPrimeira = false) {
  ...
  els.lista.innerHTML = '<div class="loading">Buscando drinks...</div>';

  estado.receitas = await buscarJson(`api/receitas?${params.toString()}`);
  els.totalReceitas.textContent = `${estado.receitas.length} drink${estado.receitas.length === 1 ? "" : "s"} na lista`;
  renderizarLista();
...
async function carregarDetalhe(nome) {
  els.detalhe.innerHTML = '<section class="loading">Carregando drink...</section>';
