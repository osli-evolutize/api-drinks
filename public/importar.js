...
function renderizarResultados() {
  if (receitasEncontradas.length === 0) {
    resultadosInternet.innerHTML = '<p class="field-hint">Nenhum drink encontrado.</p>';
    return;
  }
...
formImportar.addEventListener("submit", async (event) => {
  ...
  mostrarMensagem("Buscando drinks na internet...");
  resultadosInternet.innerHTML = '<p class="field-hint">Pesquisando paginas com drinks estruturados...</p>';
  ...
  try {
    ...
    mostrarMensagem(`${receitasEncontradas.length} drink${receitasEncontradas.length === 1 ? "" : "s"} encontrado${receitasEncontradas.length === 1 ? "" : "s"}.`, "sucesso");
...
resultadosInternet.addEventListener("click", async (event) => {
  ...
  try {
    ...
    mostrarMensagem("Drink importado com sucesso.", "sucesso");
  } catch (err) {
