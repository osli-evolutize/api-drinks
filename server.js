...
const imagensReceitasDir = path.join(publicDir, "images", "drinks");
...
const dbConfig = {
  ...
  database: process.env.DB_DATABASE || "coqueteis",
...
function usuarioAutenticado(req) {
  const token = lerCookies(req).drinksSessao;
...
function cookieSessao(token) {
  return [
    `drinksSessao=${encodeURIComponent(token)}`,
    "Path=/",
    "HttpOnly",
    "SameSite=Lax",
  ].join("; ");
...
function logout(req, res) {
  ...
  enviarJsonComHeaders(res, 200, { ok: true }, {
    "Set-Cookie": "drinksSessao=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0",
  });
}
...
async function listarUnidades(res) {
  ...
  const resultado = await pool.request().query(`
    ...
    WHERE LTRIM(RTRIM(IngredienteMedidaUnidade)) <> ''
    GROUP BY LTRIM(RTRIM(IngredienteMedidaUnidade))
    ORDER BY LTRIM(RTRIM(IngredienteMedidaUnidade));
  `);
...
async function listarReceitas(res, url) {
  ...
  const resultado = await request.query(`
    SELECT
      ...
      r.ReceitaVolume AS volume,
      r.ReceitaVolume AS peso,
      r.ReceitaAlcool AS alcool,
      0 AS calorias,
      0 AS proteinas,
      0 AS carboidratos,
      0 AS gorduras,
  ...
  for (const ingrediente of ingredientes) {
    ...
    if (resultado.recordset.length === 0) {
      const ingredienteResultado = await new sql.Request(transaction)
        ...
        .query(`
          SELECT
            IngredienteAlcool AS alcool,
            0 AS pesoMedida
          FROM dbo.Ingrediente
          WHERE LTRIM(RTRIM(IngredienteNome)) = @nome;
        `);

      resultado.recordset = ingredienteResultado.recordset.length > 0
        ? ingredienteResultado.recordset
        : [{ alcool: 0, pesoMedida: 0 }];
    }

    const base = resultado.recordset[0];
    const volume = gramasPorMedida(ingrediente.quantidade, Number(base.pesoMedida));
    const teorAlcoolico = Number(base.alcool || 0);

    resumo.volume += volume;
    resumo.alcoolPuro += volume * (teorAlcoolico / 100);
  }

  const alcool = resumo.volume > 0
    ? Math.round((resumo.alcoolPuro / resumo.volume) * 100)
    : 0;

  return {
    peso: Math.round(resumo.volume),
    volume: Math.round(resumo.volume),
    alcool,
    calorias: 0,
    proteinas: 0,
    carboidratos: 0,
...
async function criarReceita(req, res) {
  ...
  try {
    ...
    await new sql.Request(transaction)
      ...
      .input("alcool", sql.VarChar(200), String(resumo.alcool))
      .input("foto", sql.VarChar(200), "")
...
async function atualizarReceita(req, res) {
  const dados = await lerCorpoJson(req);
  const nomeOriginal = textoObrigatorio(dados.nomeOriginal || dados.nomeAnterior || dados.nome, "a receita original", 60);
  const nome = textoObrigatorio(dados.nome, "o nome da receita", 60).toUpperCase();
  const instrucoes = textoObrigatorio(dados.instrucoes, "o preparo", 8000);
  const categoriaCodigo = numeroInteiro(dados.categoriaCodigo, "Categoria");
  const pessoas = numeroInteiro(dados.pessoas, "Pessoas");
...
async function criarIngrediente(req, res) {
  ...
  const alcool = numeroInteiro(dados.alcool ?? dados.calorias ?? 0, "Alcool");
...
async function baixarTexto(url, limiteBytes = 2 * 1024 * 1024) {
  const resposta = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; DrinksBot/1.0)",
...
async function pesquisarReceitasInternet(res, url) {
  ...
  const partes = [busca, ingredientes, "drink coquetel receita"].filter(Boolean);

  if (partes.length <= 1) {
    enviarJson(res, 400, { erro: "Informe um drink, categoria ou ingrediente para pesquisar" });
    return;
  }
...
async function importarReceitaInternet(req, res) {
  ...
  try {
    ...
    await new sql.Request(transaction)
      ...
      .input("pessoas", sql.Int, pessoas)
      .input("zero", sql.Int, 0)
      .input("alcool", sql.VarChar(200), "0")
      .input("foto", sql.VarChar(200), "")
      .input("imagem", sql.VarBinary(sql.MAX), Buffer.alloc(0))
      .input("categoriaCodigo", sql.SmallInt, categoriaCodigo)
      .query(`
        ...
        VALUES (@nome, @instrucoes, @pessoas, @imagem, @categoriaCodigo, @zero, @alcool, @foto);
      `);
...
function textoCombinaComBusca(texto, busca) {
  ...
  if (encontradas.length < minimo) return false;

  return PALAVRAS_CONTEXTO_DRINK.some((palavra) => textoNormalizado.includes(palavra));
}
...
function montarConsultasImagemReceita(receita) {
  ...
  const consultas = [
    `${nome} drink`,
    `"${nome}" coquetel`,
    `${normalizado} cocktail`,
    `${normalizado} receita drink`,
  ];
...
async function pesquisarImagensCocktailDb(termo) {
  const imagens = [];
  const normalizado = String(termo || "").trim();
  if (!normalizado) return imagens;

  const urls = [
    `https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${encodeURIComponent(normalizado)}`,
    `https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=${encodeURIComponent(normalizado)}`,
  ];
    ...
    try {
      ...
      for (const drink of drinks) {
        adicionarImagemUnica(imagens, {
          ...
          fonte: `https://www.thecocktaildb.com/drink/${drink.idDrink}`,
        });
        if (imagens.length >= 18) break;
      }
    } catch {
      // Tenta a proxima consulta.
    }
...
async function pesquisarImagensReceita(res, url) {
  ...
  if (!receita) {
    enviarJson(res, 400, { erro: "Informe o drink para pesquisar imagens" });
    return;
  }
  ...
  for (const imagem of await pesquisarImagensCocktailDb(receita)) {
  ...
  if (imagens.length < 24) {
    for (const consulta of consultas.slice(0, 3)) {
      try {
        const consultaImagem = `${consulta} pronto`;
...
server.listen(port, host, () => {
  console.log(`API drinks rodando em http://${host}:${port}`);
});
